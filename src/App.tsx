...
import { Menu, Swords, Zap, ChevronRight, ChevronLeft, Home as HomeIcon, MapPin, BookOpen, Utensils } from "lucide-react";
...
function Navbar() {
  ...
  const navItems = [
    ...
    { path: "/vibe-check", label: "心情", icon: Zap },
    { path: "/the-truth", label: "真相", icon: MapPin },
    { path: "/survival-tips", label: "指南", icon: BookOpen },
    { path: "/group-fight", label: "PK", icon: Swords },
  ];
...
function Sidebar({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  const menuItems = [
    { path: "/", label: "探索推薦", icon: HomeIcon },
    { path: "/vibe-check", label: "心情探測", icon: Zap },
    { path: "/the-truth", label: "美食真相", icon: MapPin },
    { path: "/survival-tips", label: "生存指南", icon: BookOpen },
    { path: "/group-fight", label: "挑戰轉盤", icon: Swords },
  ];
...
function AppContent() {
  ...
  return (
    <div className="min-h-screen bg-sky-50/50 pb-32 selection:bg-accent selection:text-white">
      ...
      <main className="relative z-10 pt-4">
        <Routes>
          ...
          <Route path="/the-truth" element={<TheTruth />} />
          <Route path="/survival-tips" element={<SurvivalTips />} />
          <Route path="/group-fight" element={<GroupFight />} />
        </Routes>
      </main>
      <Navbar />
    </div>
